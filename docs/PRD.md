...
```
启动 → 引导问卷(5题) → AI 从 15 款车里挑 3 款生成决策报告 → 保存历史
```

> 设计演进：PRD 原方案是"选 2-3 款候选车再让 AI 对比"。实际开发中改成"AI 直接从全部车型里推荐 3 款"——
> 让用户先筛选与"我需要 AI 帮我下决心"的初衷矛盾。
...
| ~~F3~~ | ~~候选车型选择页~~ | ~~P0~~ | _已合并到 F4：AI 直接从全部车型挑 3 款_ |
| F4 | AI 决策报告页 | P0 | 调用 LLM API，从 15 款里挑 3 款生成个性化报告 |
...
> v1.0.2 起从 15 款扩到 80+ 款主流纯电（用户反馈："这 15 款太少，AI 应该看真正的全市场"）。
> 涵盖国产新势力（小鹏/蔚来/理想/小米/智界/阿维塔/极氪等）、BYD 全谱系、合资（大众 ID / 起亚 / 现代）、
> BBA 与豪华（宝马 i / 奔驰 EQ / 奥迪 e-tron / 保时捷 Taycan / Polestar / 沃尔沃）等。
...
[问卷页 SurveyScreen]
       ↓ 5 题答完
[加载页 LoadingScreen] (调用 LLM API,AI 从 15 款中挑 3 款)
       ↓ API 返回 / 本地兜底
[报告页 ReportScreen]
       ↓ 已自动保存
